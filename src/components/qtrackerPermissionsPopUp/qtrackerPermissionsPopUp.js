...
export default class QtrackerPermissionsPopUp extends Component {
    constructor(props) {
        super(props);
        this.state = {
            visible : false,
    ...
    async request(){
      ...
      if(this.state.name && this.state.description){
        ...
        let body ={
          ...
          await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/qtracker/requestPER", options)
              .then(response => response.json())
              .then(async json => {
          ...
          <Modal visible={this.state.visible} width="660px" height="640px" effect="fadeInUp" onClickAway={() => this.closeModal()}>
              ...
              <div className="qtrackerRequest__container">
                ...
                <thead>
                  <tr>
                    <th colSpan={3}>
                      <center className="qtracker__popUp__title" style={{marginBottom: "30px"}}><h3>Permissions</h3></center>
                    </th>
                  </tr>
                </thead>
                <tbody>
