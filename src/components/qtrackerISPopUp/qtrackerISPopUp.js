        ...
        return (
            <div>
                    <div className='mainmenu__item__container' style={{marginTop:"10px"}}>
                      <span style={{display:"flex"}} ><div style={{width:"280px"}}><text className='mainmenu__item' onClick={()=> this.openModal()}>Interface sending</text></div></span>
                    </div>         
                    <div>
                    ...
                    <Modal visible={this.state.visible} width="700" height="530" effect="fadeInUp" onClickAway={() => this.closeModal()}>
                        ...
                        <select id="projectSelect" className="projectSelect">
                            ...
                            <input data-for="sending-help" data-tip="sending help" data-iscapture="true" type="text" placeholder="Sending" id="sending" className="qtrackerPopUp__input__text" ref="sending" style={{marginBottom: "20px", color:'black'}} value={this.state.sending} onChange={(e) => this.setState({sending: e.target.value})} ></input>
