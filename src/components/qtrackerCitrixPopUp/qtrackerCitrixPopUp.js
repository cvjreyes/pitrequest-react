...
export default class QtrackerCitrixPopUp extends Component {
    constructor(props) {
        super(props);
        this.state = {
            visible : false,
    ...
    async request(){
      ...
      if(this.state.name && this.state.description){
        ...
        let body ={
          ...
          await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/qtracker/requestCIT", options)
              .then(response => response.json())
              .then(async json => {
      ...
      return (
        <div>
          <div className='mainmenu__item__container' style={{marginTop:"10px"}}>
            <span style={{display:"flex"}} ><div style={{width:"280px"}}><text className='mainmenu__item' onClick={()=> this.openModal()}>Citrix</text></div></span>
          </div>   
          <div>
          <Modal visible={this.state.visible} width="660px" height="640px" effect="fadeInUp" onClickAway={() => this.closeModal()}>
              ...
              <div className="qtrackerRequest__container">
                ...
                <thead>
                  <tr>
                    <th colSpan={3}>
                      <center className="qtracker__popUp__title" style={{marginBottom: "30px"}}><h3>Citrix</h3></center>
                    </th>
                  </tr>
                </thead>
                <tbody>
