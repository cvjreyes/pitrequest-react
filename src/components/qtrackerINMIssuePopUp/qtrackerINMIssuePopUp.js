...
export default class QtrackerINMIssuePopUp extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
      name: secureStorage.getItem("user"),
      attach: null,
      description: null,
    ...
    await fetch(
      ...
      .then(async (json) => {
        ...
        for (let i = 0; i < json.projects.length; i++) {
            if (json.projects[i].name.includes("IM_")) {
        ...
        >
          <span style={{ display: "flex" }}>
            <div style={{ width: "280px" }}>
              <text className="mainmenu__item" onClick={() => this.openModal()}>
                Issue Incident
              </text>
            </div>
          </span>
          ...
          >
            <div
              className={`alert alert-success ${
                this.state.errorBlankRequest ? "alert-shown" : "alert-hidden"
              }`}
              onTransitionEnd={() =>
                this.setState({ errorBlankRequest: false })
              }
            >
              <AlertF
                type="qtracker"
                text="At least one field is blank!"
                margin="5px"
              />
            </div>
            <div className="qtrackerRequest__container">
              <table>
                <thead>
                  <tr>
                    <th colSpan={3}>
                      <center
                        className="qtracker__popUp__title"
                        style={{ marginBottom: "30px" }}
                      >
                        <h3>Issue Incident</h3>
                      </center>
                    </th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <tr>
                    ...
                    <td style={{ textAlign: "left" }}>
                      <label className="priority__label" for="prioritySelect">
                        Priority
                      </label>
                    </td>
                  </tr>
                  ...
                  {/* Segunda fila: Description */}
                  <tr>
                    <td style={{ textAlign: "left" }}>
                      <label className="priority__label" for="description">
                        Description
                      </label>
                    </td>
                  </tr>
              ...
              <button class="btn__submit" onClick={() => this.request()}>
                Submit
              </button>
              <button class="btn__cancel" onClick={() => this.closeModal()}>
                Cancel
              </button>
            </div>
          </Modal>
