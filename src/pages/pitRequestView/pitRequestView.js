...
const PitRequestView = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then(async (json) => {
        if (json.urgent > 0) {
          setAlertComponentCount(
            <div className={`alert alert-success alert-shown`}>
              <AlertF
                type="warning"
                text={`You have ${json.urgent} urgent incidences!`}
                margin="-27px"
              />
            </div>
          );
  ...
  useEffect(() => {
    ...
    if (secureStorage.getItem("role") === "3D Admin") {
      //Si el user es 3d admin tiene la opcion de guardar cambios y acceder a la ventana de usuarios
      setSaveBtn(
      ...
      setUsersButton(
        ...
        >
          ...
          ></img>
          <p className="navBar__button__text">Users</p>
        </button>
      );
    } else {
      setSaveBtn(null);
  ...
  useEffect(async () => {
    ...
    await fetch(
      ...
      .then(async (json) => {
        let counter = [
          { name: "Pending", value: json.pending },
          { name: "In progress", value: json.progress },
          { name: "Accepted", value: json.accepted },
          { name: "Rejected", value: json.rejected },
          { name: "Materials", value: json.materials },
          { name: "Ready to Load", value: json.readytoload },
        ];
  ...
  useEffect(async () => {
    //Si se activa el toggle de showAll se muestran todas las incidencias, no solo las que estan abiertas
    // console.log("show all use effect: " + showAll);
    setContent(
      <QTrackerViewDataTable
        updateObservations={updateObservations.bind(this)}
        updateHours={updateHours.bind(this)}
        updateData={updateData}
        updateStatus={updateStatus.bind(this)}
        updatePriority={updatePriority.bind(this)}
  ...
  useEffect(async () => {
    if (currentRole === "3D Admin") {
      //Si el user es 3d admin
      if (currentTab === "View") {
        //Si el tab actual es la vista
        ...
        setProjectsButton(
          ...
          >
            <img src={FolderIcon} alt="pro" className="navBar__icon"></img>
            <p className="navBar__button__text">Projects</p>
          </button>
        );
        setBackToMenuButton(
          <button
          ...
          >
            <img
              ...
              style={{ marginRight: "0px" }}
            ></img>
            <p className="navBar__button__text">Back</p>
          </button>
        );
        ...
        setBuscadorProyecto(
          <div>
            ...
            >
              ...
              <p className="navBar__button__text">Buscador</p>
            </button>
          </div>
        );
          ...
          <div style={{ display: "flex", float: "right" }}>
            <div style={{ display: "inline-block" }}>
              <label
                ...
                style={{ marginLeft: "-280px", marginRight: "230px" }}
              >
                Project:{" "}
              </label>
              <select
              ...
              <button
                className="navBar__button__buscador"
                onClick={() => saveChanges()}
              >
                <FontAwesomeIcon
                  className="navBar__icon__buscador"
        ...
        //Si es la tabla de usuarios
        secureStorage.setItem("tab", "Users");
        setExportUsersReport(
          <button
            className="action__btn"
            name="export"
            value="export"
        ...
        setContent(
          <UsersDataTable
            ...
            submitProjects={submitProjects.bind(this)}
          />
        );
        setBackToMenuButton(
          <button
        ...
        setRequestAccessButton(
          ...
          >
            <img
              ...
              style={{ marginRight: "0px" }}
            ></img>
            <p className="navBar__button__text">Access requests</p>
          </button>
        );
        ...
        //Si es la tabla de gestion de peticiones de acceso
        ...
        setContent(
          <RequestAccessDataTable
            ...
            acceptRequest={acceptRequest.bind(this)}
      ...
      //Si no es 3d admin se muestra la vista de incidencias normal
      setContent(
        <QTrackerViewDataTable
          updateObservations={updateObservations.bind(this)}
          updateHours={updateHours.bind(this)}
      ...
      .then(async (json) => {
        ...
        for (let i = 0; i < json.rows.length; i++) {
          row = {
            ...
            project: json.rows[i].project,
            carta: json.rows[i].carta,
            user: json.rows[i].user,
            created_at:
              json.rows[i].created_at.toString().substring(0, 10) +
              " " +
              json.rows[i].created_at.toString().substring(11, 19),
            ...
            hours: json.rows[i].hours,
            admin: json.rows[i].admin,
            ar_date: json.rows[i].accept_reject_date,
          };
          ...
          if (json.rows[i].accept_reject_date) {
            row.ar_date =
              json.rows[i].accept_reject_date.toString().substring(0, 10) +
              " " +
              json.rows[i].accept_reject_date.toString().substring(11, 19);
          }
            ...
            await fetch(
              ...
              .then(async (json) => {
                ...
                for (let i = 0; i < json.rows.length; i++) {
                  ...
                  if (json.rows[i].accept_reject_date) {
                    row.ar_date =
                      json.rows[i].accept_reject_date
                        .toString()
                        .substring(0, 10) +
                      " " +
                      json.rows[i].accept_reject_date
                          ...
                          .then(async (json) => {
                            ...
                            for (let i = 0; i < json.rows.length; i++) {
                              row = {
                                ...
                                hours: json.rows[i].hours,
                                admin: json.rows[i].admin,
                                ar_data: json.rows[i].accept_reject_date,
                              };
                                      ...
                                      .then(async (json) => {
                                        ...
                                        for (
                                          ...
                                          if (json.rows[i].status === 0) {
                                            ...
                                            json.rows[i].status === 1
                                          ) {
                                            row.status = "In progress";
                                          } else if (
                                            json.rows[i].status === 2
                                          ) {
                                            row.status = "Ready";
                                          } else if (
                                            json.rows[i].status === 3
                                          ) {
                                            row.status = "Rejected";
                                          } else if (
                                            json.rows[i].status === 4
                                          ) {
                                            row.status = "Materials";
                                          } else if (
                                            json.rows[i].status === 5
                                          ) {
                                            row.status = "Ready to Load";
                                          }
  ...
  async function changeRole(value) {
    //Al cambiar el rol cambiamos la vista
  ...
  async function rejectRequest(id) {
    //Rechazar una request por su id
    let body = {
      id: id,
      user: currentUser,
    };
    ...
    <div>
      ...
      <div
        className={`alert alert-success ${
          success ? "alert-shown" : "alert-hidden"
        }`}
      ...
      <div
        className={`alert alert-success ${
          error ? "alert-shown" : "alert-hidden"
        }`}
        onTransitionEnd={() => setError(false)}
      >
        <AlertF
          type="error"
          subtext="A problem occurred while saving changes!"
        />
      </div>
      <div
        className={`alert alert-success ${
          notReadyWarning ? "alert-shown" : "alert-hidden"
        }`}
      ...
      <table className="isotracker__table__container">
        <tbody>
          ...
          >
            ...
            >
              <div
                style={{ height: dataTableHeight, width: "2200px" }}
