...
const PipingExpansionJoins = () => {
    ...
    useEffect(async()=>{
        ...
        await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/getProjectsByEmail/" + secureStorage.getItem("user"), options)
            ...
            .then(async json => {
                ...
                await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/getInstExpansionJoinsByProject/" + currentProject, options)
                .then(response => response.json())
                .then(async json => {
    ...
    async function saveChanges(){
        ...
        await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/submitInstExpansionJoins", options)
        .then(response => response.json())
        .then(async json =>{
            if(json.success){
    ...
    async function downloadReport(){
        await fetch("http://"+process.env.REACT_APP_SERVER+":"+process.env.REACT_APP_NODE_PORT+"/downloadInstsExpansionJoinsByProject/" + currentProject)
        .then(response => response.json())
        .then(json => {
            let rows = JSON.parse(json)

            const headers = ["Tag", "Spec", "P1Bore", "P2Bore", "Rating", "End Preparation", "Iso Description", "Ident", "Face to face", "FLG Short Code", "Request date", "Request to load date", "Ready in E3D date", "Comments", "Ready to Load", "Ready in 3D", "Updated"]
            ...
            const header_cells = ['A1', 'B1', 'C1', 'D1', 'E1', 'F1', 'G1', 'H1', 'I1', 'J1', 'K1', 'L1', 'M1', 'N1', '01', 'P1', 'Q1']
    ...
    let table = <CSPTrackerExpansionJoinsDataTable currentRole = {currentRole} updateDataMethod = {updateDataMethod.bind(this)} updateData = {updateData} uploadDrawingSuccess = {uploadSuccess.bind(this)} updateDrawingSuccess = {updateSuccess.bind(this)} drawingUploadError={drawingUploadError.bind(this)} currentProject={currentProject}/>
    ...
    if(currentTab === "View"){
        table = <CSPTrackerExpansionJoinsDataTable currentRole = {currentRole} updateDataMethod = {updateDataMethod.bind(this)} updateData = {updateData} uploadDrawingSuccess = {uploadSuccess.bind(this)} updateDrawingSuccess = {updateSuccess.bind(this)} drawingUploadError={drawingUploadError.bind(this)} currentProject={currentProject}/>
        ...
        if(currentRole === "Design"){
            cols = [{ data: "tag", type:'dropdown'}, {data: "spec", type:'dropdown'}, {data: "p1bore", type:'dropdown'}, {data: "p2bore", type:"dropdown"}, {data: "rating", type:"text"}, {data:"end_prep", type:"text"}, {data:"iso_desc", type:"text"}, {data:"ident", type:"text", readOnly:true}, {data:"facetoface", type:"text"}, {data:"flg_code", type:"text", readOnly:true}]
            ...
            cols = [{ data: "tag", type:'dropdown', readOnly:true}, {data: "spec", type:'dropdown', readOnly:true}, {data: "p1bore", type:'dropdown', readOnly:true}, {data: "p2bore", type:"dropdown", readOnly:true}, {data: "rating", type:"text", readOnly:true}, {data:"end_prep", type:"text", readOnly:true}, {data:"iso_desc", type:"text", readOnly:true}, {data:"ident", type:"text"}, {data:"facetoface", type:"text", readOnly:true}, {data:"flg_code", type:"text"}]
            for(let i = 0; i < editData.length; i++){
                cells.push({
                    row: i,
                    col: 0,
        ...
        table = <HotTable
        data={editData}
        colHeaders = {["Tag", "Spec", "P1Bore", "P2Bore", "Rating", "End Preparation", "Iso Description", "Ident", "Face to face", "FLG Short Code", "Request date", "Request to load date", "Ready in E3D date", "Comments", "Ready to Load", "Ready in 3D", "Updated"]}        
        rowHeaders={true}
        width="2170"
        height="530"
        settings={settings} 
